	...
	# first Khmer goal: build four Buddhist and Hindu monasteries and Wat Preah Pisnulok by 1200 AD
		...
		if isPossible(iKhmer, 0):
			if iBuilding in [iWatPreahPisnulok, iBuddhistMonastery, iHinduMonastery]:
				...
				bAngkorWat = sd.getWonderBuilder(iWatPreahPisnulok) == iKhmer
...
def getUHVHelp(iPlayer, iGoal):
	...
	if iPlayer == iEgypt:
		...
		if iGoal == 0:
			bConstruction = sd.getFirstDiscovered(iConstruction) == iBabylonia
			bArithmetics = sd.getFirstDiscovered(iArithmetics) == iBabylonia
			bWriting = sd.getFirstDiscovered(iWriting) == iBabylonia
			bCalendar = sd.getFirstDiscovered(iCalendar) == iBabylonia
			bContract = sd.getFirstDiscovered(iContract) == iBabylonia
		...
		if iGoal == 0:
			bGeometry = sd.getFirstDiscovered(iGeometry) == iGreece
			bTradition = sd.getFirstDiscovered(iTradition) == iGreece
			bAesthetics = sd.getFirstDiscovered(iAesthetics) == iGreece
			bPhilosophy = sd.getFirstDiscovered(iPhilosophy) == iGreece
			bMedicine = sd.getFirstDiscovered(iMedicine) == iGreece
		...
		if iGoal == 0:
			...
			bArchitecture = sd.getFirstDiscovered(iArchitecture) == iRome
			bPolitics = sd.getFirstDiscovered(iPolitics) == iRome
	...
	# Maya goals have no stages
		...
		if iGoal == 0:
			...
			bThreeGorgesDam = sd.getWonderBuilder(iItaipuDam) == iBrazil
